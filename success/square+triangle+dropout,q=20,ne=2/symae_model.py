...
class SymAE_Dense(tf.keras.Model):
    def __init__(self,N=1000,nt=20,d=100,p=8,q=22,kernel_size=5,filters=64,dropout_rate=0.65): 
        ...
        nui_enc_input=tfk.Input(shape=(nt, d), dtype='float32', name='sym_enc_input')
        nui_enc_output = symae.NuisanceEncoderDense1D(q, d, [2, 2, 2])(nui_enc_input)
        ...
        mixer_output=symae.MixerDense1D(d, p+q)(mixer_input)
        ...
        latentcat=symae.LatentCat(dropout_rate)
    ...
    def model(self, x):
        return tfk.Model(inputs=x, outputs=self.call(x), name='symae_dense')
