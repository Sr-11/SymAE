...
class NuisanceEncoder1D(tf.keras.Model):
  def __init__(self, kernel_size, filter,  fstep=[2,4,8], tdown=[1,1,2,2], latent_dim=512):
    ...
    self.d1 = tf.keras.layers.Dense(80, activation='relu')
    self.d2 = tf.keras.layers.Dense(80, activation='relu')
    self.d3 = tf.keras.layers.Dense(60, activation='relu')
    self.d4 = tf.keras.layers.Dense(60, activation='relu')
    self.d5 = tf.keras.layers.Dense(latent_dim, activation='relu')
